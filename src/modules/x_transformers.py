...
class LayerScale(Module):
    def __init__(self, fn: Module, dim, init_value=0.0, unit_offset=False):
        super().__init__()
        self.unit_offset = unit_offset
...
class AdaptiveLayerScale(Module):
    def __init__(self, fn: Module, dim, dim_condition=None, init_bias_value=-2.0):
        super().__init__()
        self.fn = fn
...
class Attention(Module):
    ...
    def forward(
        ...
        b, n, h, kv_h, head_scale, num_mem_kv, device, has_context = (
            x.shape[0],
            x.shape[1],
            self.heads,
            self.kv_heads,
        ...
        k, v, r = tuple(
            maybe(rearrange)(t, "b n (h d) -> b h n d", h=kv_h) for t in (k, v, r)
        )
        ...
        if exists(value_residual):
            if self.neutreno_value_residual:
                ...
                v = v * value_residual_mix + value_residual * (1.0 - value_residual_mix)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert at_most_one_of(alibi_pos_bias, rel_pos_bias, data_dependent_alibi), (
            "you can only choose one of Alibi positional bias, data dependent Alibi (forgetting transformers), or T5 relative positional bias"
        )
        ...
        assert at_most_one_of(sandwich_norm, resi_dual), (
            "either sandwich norm or resiDual is selected, but not both"
        )
...
class PrefixDecoder(AttentionLayers):
    ...
    def forward(self, x, *args, attn_mask=None, prefix_attn_len=None, **kwargs):
        ...
        if exists(prefix_attn_len):
            ...
            prefix_mask = torch.arange(n, device=device) < rearrange(
...
class TransformerWrapper(Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        b, n, device, num_mems, has_memory_tokens, emb_frac_gradient, orig_mask = (
            x.shape[0],
            x.shape[1],
            x.device,
        ...
        pos_emb = (
            self.pos_emb(x, pos=pos, seq_start_pos=seq_start_pos)
