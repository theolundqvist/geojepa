...
struct utils {
        ...
        static std::vector<point> intersection_area(const std::vector<point> &nodes, const tileZXY &tile, const bool is_polygon) {
                const auto [lat1, lon1, lat2, lon2] = tileZXYToLatLonBBox(tile);
                const std::vector<point> tile_bbox{{lat1, lon1}, {lat1, lon2}, {lat2, lon2}, {lat2, lon1}, {lat1, lon1}};
        ...
        static ecef_t geodeticToECEF(const xy_t &geo) {
        ...
        static ENUCoord ecefToENU(const ecef_t &target, const ecef_t &origin, const xy_t origin_latlon) {
            const double dx = target.x - origin.x;
            const double dy = target.y - origin.y;
            const double dz = target.z - origin.z;
            const double lon0 = origin_latlon.x() * M_PI / 180.0;
            const double lat0 = origin_latlon.y() * M_PI / 180.0;
        ...
        static ENUCoord pointToENU(const xy_t &target, const xy_t &origin) {
            const ecef_t target_ecef = geodeticToECEF(target);
            const ecef_t origin_ecef = geodeticToECEF(origin);
            return ecefToENU(target_ecef, origin_ecef, origin);
        }

        static void normalize(unprocessed::Tile *tile) {
            const auto tile_zxy = utils::tileZXY(tile->zoom(), tile->x(), tile->y());
            const auto [lat1, lon1, lat2, lon2] = utils::tileZXYToLatLonBBox(tile_zxy);
            const auto [min_lat, max_lat] = std::minmax(lat1, lat2);
            const auto [min_lon, max_lon] = std::minmax(lon1, lon2);
        ...
        static std::tuple<std::vector<point>, double, double> subdivide(const int divisions) {
                std::vector<point> midpoints;{};

                const double lat_dist = 1.0/(divisions+1);
                const double lon_dist = 1.0/(divisions+1);

                for(int y = 0; y < divisions; ++y) {
                        for(int x = 0; x < divisions; ++x) {
                                midpoints.emplace_back(lat_dist/2 + lat_dist * y,lon_dist/2 + lon_dist * x);
                        }
                }
                return {midpoints, lat_dist, lon_dist};

        }

        static float parse_speed(const std::string& speed_str) {
                std::stringstream ss(speed_str);
                float speed{0};
                std::string mph{};
                if (ss >> speed) {
                        if (ss >> mph && mph == "mph") {
                ...
                if (speed_str == "US:urban") {
                        return 40; //Assume US:urban is 25mph=40kmh
                }
                return speed;
        }

        static bool match_substring(const std::string& target, const std::string& substr) {
