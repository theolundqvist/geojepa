...
struct utils {
    ...
    template<typename T>
    static void check_nan(const T &value) {
        if (std::isnan(value)) {
    ...
    static tileZXY scale_tileZXY(const tileZXY &tile, const size_t new_zoom) {
        const auto [lat, lon] = tileZXYToLatLon(tile);
        return latLonToTile(lat, lon, new_zoom);
    }
    ...
    static auto rotated_point_fn(const double radians) {
        const auto sin = std::sin(radians);
        const auto cos = std::cos(radians);
        auto rotate = [sin, cos](const point p) {
            double new_x = p.lat * cos - p.lon * sin;
            double new_y = p.lat * sin + p.lon * cos;
...
//    static std::vector<point> get_AABB(const std::vector<point> &points) {
//        double min_lat = -MAXFLOAT;
//        double max_lat = MAXFLOAT;
//        double min_lon = -MAXFLOAT;
//        double max_lon = MAXFLOAT;
//        for (const auto &p: points) {
...
//    }
    ...
    static min_box_result random_box(const std::vector<point> &nodes, float side) {
        const auto p = nodes[0];
        auto min = -side/2.0;
        auto max = side/2.0;
        std::uniform_real_distribution<> dis(0.0, M_PI);
        auto rotation = dis(gen);
        auto rotate = rotated_point_fn(rotation);

        std::vector<utils::point> points{{min, min},
                                         {max, min},
                                         {max, max},
                                         {min, max}};
    ...
    static min_box_result min_box(
            const std::vector<point> &nodes,
            const int segments,
            const float min_side = 0.01

    ) {
        if (nodes.empty()) {
            return {.points = {}};
        } else if (nodes.size() == 1) {
            return random_box(nodes, min_side);
        }
        ...
        bg::model::box<xy_t> AABB{};
        bg::envelope(hull, AABB);
        auto min = AABB.min_corner();
        auto max = AABB.max_corner();
        if (bg::distance(min, max) < min_side) {
            return random_box(nodes, min_side);
        }
        // Calculate center-point
        const point center_point = (point(AABB.min_corner()) + point(AABB.max_corner())) / 2;
        ...
        std::vector<std::pair<std::vector<point>, float>> candidates;
        for (int n = 0; n < segments; n++) {
            std::vector<point> candidate;
            const auto angle = boost::geometry::math::pi<float>() / (float) n;

            // cache sin and cos calculations
            auto rotate = rotated_point_fn(angle);

            // rotate the hull and add as candidate
            candidate.reserve(normalized_hull.size());
            for (auto &pt: normalized_hull) {
                candidate.push_back(rotate(pt));
            }
        ...
        const auto width = max_corner.lon - min_corner.lon;
        const auto height = max_corner.lat - min_corner.lat;
        std::vector<point> min_box_points = {
                min_corner,
                {min_corner.lat, max_corner.lon},
                max_corner,
                {max_corner.lat, min_corner.lon},
        };

        auto width_mult = 1.0;
        auto height_mult = 1.0;
        if (width < min_side) width_mult = min_side / width;
        if (height < min_side) height_mult = min_side / height;

        auto rotate = rotated_point_fn(min_angle);
        for (auto &p: min_box_points) {
            check_nan(p.lat);
            check_nan(p.lon);
            p.lat *= height_mult;
            p.lon *= width_mult;
            ...
            p.lat = rot.lat;
            p.lon = rot.lon;
            p.lat += center_point.lat;
            p.lon += center_point.lon;
            check_nan(p.lat);
            check_nan(p.lon);
        }

        return {
                .points = min_box_points,
                .area = (float) smallest_area,
                .width = (float) width,
                .height = (float) height,
                .rotation = (float) min_angle,
        };
    }
};
